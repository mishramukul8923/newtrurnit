...
const Humanizer = ({ fetchData }) => {
    ...
    const fetchRewriteData = async () => {
        setLoading(true);
        setHumanizedData(''); // Clear current data to show loader
        try {
            ...
            setHumanizedData(result.content[0].text); // Sets the new humanized data in the state
        } catch (err) {
            setError(err.message);
        } finally {
            setLoading(false); // Stop loader once data is received
        }
    };
    ...
    const handleSaveContent = async () => {
        ...
        try {
            const response = await fetch('/api/humanizedContent', {
                ...
                body: JSON.stringify({ text: text, humanizedContent: humanizedData, user_id: user_id, email: email }),
            });
    ...
    // Handle "copy click" button click
    const handleCopyClick = () => {
        navigator.clipboard.writeText(humanizedData)
            .then(() => toast.success("Text copied to clipboard!"))
            .catch((err) => console.error("Failed to copy text: ", err));
    };


    // New function to handle regenerate click
    const handleRegenerateClick = async () => {
        setError('');
        await fetchRewriteData(); // Re-fetch to generate new humanized content
    };
