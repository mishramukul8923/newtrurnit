...
const Generate = ({fetchData}) => {
   ...
   const [plan_id, setPlan_id] = useState("");
    ...
    const fetchHisrotyData = async () => {
        ...
        if (createdId && title) {
            console.log("created and title get in generate component",title)
            setText(title);
            ...
            if (!response.ok) {
                ...
                // console.log("this is generated hist", filteredData[0]?.generatedContent)
                }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFineData(result.content[0].text);
            setLoading(false);
    ...
    const handleSaveContent = async () => {
        ...
        try {
            const response = await fetch('/api/generatedContent', {
                ...
                body: JSON.stringify({
                    text: nextInput ? nextInput : text, generatedContent: fineData,
                    user_id: user_id,
                    email: email
                }),
            });
        ...
        <div className={styles.turnitGencontainer}>
            {isFirstContainerVisible ? (
                ...
                <div className={styles.turnitGencontainerSecond}>
                    <div className={styles.turnitGenPaste}>
                        <h2 className={styles.heading}>
                            <Image src="/images/generate-energy.svg" alt="Open Menu" fluid />
                            Generate
                        </h2>
